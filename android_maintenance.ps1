...
switch ($Command) {
    ...
    "debug" {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "✅ Debug APK built ($($apk.Count) files)" -ForegroundColor Green
        }
    }

    "release" {
        Write-Host "🚀 Building release AAB..." -ForegroundColor Magenta
        Write-Host "⚠️  Note: Release build requires signing key" -ForegroundColor Yellow
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "✅ Release AAB built ($($aab.Count) files)" -ForegroundColor Green
        } else {
            Write-Host "⚠️  Release build failed - missing signing key" -ForegroundColor Yellow
        }
    }
    ...
    "doctor" {
        Write-Host "👨‍⚕️ Running Android Build Doctor..." -ForegroundColor Magenta
        ...
        Write-Host "`n✅ Build System Health Check Complete" -ForegroundColor Green
    }
}