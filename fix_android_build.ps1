...
if ($CleanOnly) {
    Write-Host "`n✅ Clean-only operation completed!" -ForegroundColor Green
    Write-Host "All caches have been cleared. You can now run builds with a clean state." -ForegroundColor Yellow
    exit 0
}
...
if ($DryRun) {
    ...
    $buildTypes = @()
    if ($BuildDebug -or (-not $BuildRelease)) {  # Default to debug if neither specified
        $buildTypes += "assembleDebug"
    }
    if ($BuildRelease) {
        $buildTypes += "bundleRelease"
    }

    foreach ($buildType in $buildTypes) {
        Write-Colored "🏗️  Building: $buildType" $BLUE
        Write-Host "This may take 5-10 minutes..." -ForegroundColor Yellow
        Log-Message "Starting $buildType build with --no-daemon flag"

        try {
            Write-Host "🔧 Running: ./gradlew $buildType --no-daemon --stacktrace" -ForegroundColor Cyan

            # Prepare output file names
            $outputFile = "build_output_$buildType.txt"
            $errorFile = "build_error_$buildType.txt"

            # Execute the build command and capture output
            $buildProcess = Start-Process -FilePath ".\gradlew.bat" -ArgumentList $buildType, "--no-daemon", "--stacktrace" -NoNewWindow -Wait -PassThru -RedirectStandardOutput $outputFile -RedirectStandardError $errorFile

            # Check if the build was successful
            if ($buildProcess.ExitCode -eq 0) {
                Write-Colored "✅ $buildType BUILD SUCCESSFUL!" $GREEN
                Log-Message "$buildType build completed successfully"

                # Look for output files based on build type
                ...
                $outputFiles = Get-ChildItem -Path $outputDir -Filter $filePattern -ErrorAction SilentlyContinue
                if ($outputFiles) {
                    foreach ($file in $outputFiles) {
                        $sizeMB = [math]::Round($file.Length / 1MB, 2)
                        Write-Colored "📱 $buildType Output: $($file.Name) ($sizeMB MB)" $GREEN
                        Log-Message "Generated $buildType output: $($file.FullName) ($sizeMB MB)"
                    }
                } else {
                    Write-Colored "⚠️  $buildType output files not found in expected location" $YELLOW
                    Log-Message "Warning: $buildType output files not found in expected location"

                    # Search more broadly for output files
                    $allOutputs = Get-ChildItem -Path "app\build\outputs" -Filter $filePattern -Recurse -ErrorAction SilentlyContinue
                    if ($allOutputs) {
                        Write-Colored "📱 Found $buildType outputs in other locations:" $GREEN
                        foreach ($file in $allOutputs) {
                            $sizeMB = [math]::Round($file.Length / 1MB, 2)
                            Write-Colored "   - $($file.FullName) ($sizeMB MB)" $GREEN
                            Log-Message "Found $buildType output: $($file.FullName) ($sizeMB MB)"
                        }
                    }
                }
            } else {
                Write-Colored "❌ $buildType BUILD FAILED!" $RED
                Log-Message "$buildType build failed with exit code: $($buildProcess.ExitCode)"

                # Display error information
                if (Test-Path $errorFile) {
                    $errors = Get-Content $errorFile -Tail 50
                    Write-Host "Last 50 lines of $buildType error output:" -ForegroundColor Red
                    $errors | ForEach-Object { Write-Host $_ -ForegroundColor Red }
                }

                Write-Host "`nFor detailed error information, check $errorFile" -ForegroundColor Yellow
                Write-Host "You can also run './gradlew $buildType --info --stacktrace' manually for more details" -ForegroundColor Yellow

                # Copy output files to main log
                if (Test-Path $outputFile) {
                    Get-Content $outputFile | Out-File -Append -FilePath "build_log.txt"
                }
                if (Test-Path $errorFile) {
                    Get-Content $errorFile | Out-File -Append -FilePath "build_log.txt"
                }
            ...
            Write-Colored "❌ $buildType BUILD PROCESS ERROR: $_" $RED
            Log-Message "$buildType build process error: $_"
...
if ($DryRun) {
    ...
    if ($BuildDebug -or (-not $BuildRelease)) {
        Write-Host "   • Ran assembleDebug build" -ForegroundColor White
    }
    if ($BuildRelease) {
        Write-Host "   • Ran bundleRelease build" -ForegroundColor White
    }
}
...
Write-Host "   1. If build succeeded: Find your APK/AAB in app/build/outputs/" -ForegroundColor White
Write-Host "   2. If build failed: Check build_error_*.txt for details" -ForegroundColor White
...
Write-Host "   5. Use -BuildDebug or -BuildRelease flags for specific builds" -ForegroundColor White
