...
if ($DryRun) {
    ...
    try {
        Write-Host "🔧 Running: ./gradlew assembleDebug --no-daemon --info --stacktrace" -ForegroundColor Cyan

        # Execute the build command and capture output
        $buildProcess = Start-Process -FilePath ".\gradlew.bat" -ArgumentList "assembleDebug", "--no-daemon", "--info", "--stacktrace" -NoNewWindow -Wait -PassThru -RedirectStandardOutput "build_output.txt" -RedirectStandardError "build_error.txt"
        ...
        if ($buildProcess.ExitCode -eq 0) {
            ...
            Log-Message "Build failed with exit code: $($buildProcess.ExitCode)"
